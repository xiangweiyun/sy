...
public class CaptchaUtils {
	...
	static {
		try {
			InputStream is = CaptchaUtils.class.getClassLoader().getResourceAsStream("captcha/WellrockSlabBold.ttf");
			if (BlankUtils.isNotBlank(is)) {
				BASE_FONT = Font.createFont(Font.TRUETYPE_FONT, is);
			} else {
				logger.info("font file is null");
			}
	...
	public static Captcha createCaptcha(int WITDH, int HEIGHT) {
		// 生成一张图片
		BufferedImage image = new BufferedImage(WITDH, HEIGHT, BufferedImage.TYPE_INT_RGB);
		// 为图片构建一只画笔
	...
	private static String addRandomWord(Graphics2D g, int WITDH, int HEIGHT) {
		...
		// 设置字体
		if (BlankUtils.isNotBlank(BASE_FONT)) {
